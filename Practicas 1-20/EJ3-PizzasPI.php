...
class PizzaPi
{
    ...
    private $sauce_per_pizza = 125;
    

    public function calculateDoughRequirement(int $num_pizzas, int $additional_eaters)
    {
       return $num_pizzas * (($additional_eaters * $this->additional)+ $this->base);
    }
    ...
    public function calculateCheeseCubeCoverage(int $cheese_dimension, float $thickness, int $diameter )
    {
        ...
        $result = ($cheese_dimension ** 3)/($thickness * M_PI * $diameter);
        return intval($result);
    }

    public function calculateLeftOverSlices($pizzas, $commensals)
    {
        $portions = $pizzas * 8;
        return ($portions%$commensals);
    }
}